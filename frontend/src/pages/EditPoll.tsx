...
export function EditPoll() {
  ...
  useEffect(() => {
    if (poll) {
      ...
      setAllowStatsView(poll.allowStatsView);
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold">{t('edit_poll')}</h1>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6 space-y-4">
        ...
        <label className="flex items-center gap-2">
          <input type="checkbox" checked={allowStatsView} onChange={(e) => setAllowStatsView(e.target.checked)} className="rounded" />
      ...
      <div className="space-y-4">
        ...
        {questions.map((q, qi) => (
          <div key={qi} className="bg-white rounded-lg border border-gray-200 p-4 space-y-3">
            <div className="flex items-center justify-between">
              ...
              {questions.length > 1 && (
                <button onClick={() => removeQuestion(qi)} className="text-red-500 text-sm">✕</button>
              )}
            </div>
            <input
              type="text"
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-1 text-sm">
                <input type="radio" checked={q.type === 'single'} onChange={() => updateQuestion(qi, 'type', 'single')} />
                {t('single_choice')}
              </label>
              <label className="flex items-center gap-1 text-sm">
                <input type="radio" checked={q.type === 'multi'} onChange={() => updateQuestion(qi, 'type', 'multi')} />
                {t('multi_choice')}
              </label>
            </div>
            <div className="space-y-2">
              {q.options.map((o, oi) => (
                <div key={oi} className="flex items-center gap-2">
                  ...
                  />
                  {q.options.length > 2 && (
                    <button onClick={() => removeOption(qi, oi)} className="text-red-400">✕</button>
                  )}
                </div>
              ))}
              {q.options.length < 5 && (
                <button onClick={() => addOption(qi)} className="text-sm text-blue-600 hover:underline">
                  {t('add_option')}
                </button>
              )}
            </div>
        ...
        <button onClick={addQuestion} className="w-full py-2 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:border-blue-400">
          {t('add_question')}
        </button>
      </div>

      <button onClick={handleSubmit} disabled={saving} className="w-full py-3 bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 disabled:opacity-50">
        {t('save')}
      </button>
    </div>
  );
